...
import ndm_util
from ndm_util import NDM_DIR, NDM_BIN, NDM_NS, NDM_TEST_YAML_NAME, NDM_TEST_YAML_PATH

import k8s_util
from k8s_util import POD_WAIT_STATES, POD_GOOD_STATES, POD_GOOD_PHASES, POD_BAD_PHASES
from k8s_util import NS_GOOD_PHASE
...
def wait_till_default_ns_is_ready(max_try=1, wait_for_sec=1):
    """
    This method busy waits until default namespace is ready or number of try exceeds max_try.
    Each try is make after waiting for wait_for_sec number of seconds.

    :param int max_try: maximum number of tries to be make to check if NDM_NS is up.
                        (default to 1) (minimum value 1)
    ...
    while i < max_try:
        try:
            namespaces = k8s_util.get_all_namespaces_dict()
            if NDM_NS in namespaces:
                if namespaces[NDM_NS].status.phase in NS_GOOD_PHASE:
                    break
                else:
                    print "Waiting as Namespace '" + NDM_NS + "' is in '"\
                        + namespaces[NDM_NS].status.phase + "' phase."
            else:
                print "Waiting as Namespace '" + NDM_NS + "' is not yet there."
        except K8SApiException as err:
            if err.status == 404:
                print 'Status code: 404 while connecting to REST API. Error:', str(err)
            else:
                print 'K8SApiException occured',\
                    'while checking if Namespace is up. Error:', str(err)
        except Exception as err:
            print 'Error occured while checking if Namespace is up. Error:', str(err)
        finally:
            i += 1
            sleep(wait_for_sec)
    print "Namespace '" + NDM_NS + "' is ready."
...
def validate_log(max_try=0, wait_for_sec=1):
    ...
    if ndm_pod.status.phase in POD_GOOD_PHASES:
        ...
        while i < max_try:
            try:
                ndm_log = k8s_util.get_log(ndm_pod.metadata.name, NDM_NS)
                break
            except K8SApiException as err:
