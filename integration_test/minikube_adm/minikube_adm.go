...
func init() {
	...
	if useSudoEnv == "true" { // If it is mensioned in the environment variable to use sudo
		useSudo = true // use sudo then
	} else if useSudoEnv == "false" { // Else if it is mensioned in the environment variable not to use sudo
		useSudo = false // do not use sudo
	} // Else use default value mensioned above
...
// StartMinikube method starts minikube with `--vm-driver=none` and
// `--feature-gates=MountPropagation=true` options.
func StartMinikube() {
	err := runCommand("minikube start --vm-driver=none --feature-gates=MountPropagation=true")
