...
func GetNodes() (nodeNames []core_v1.Node, err error) {
	...
	waited := 0
	for waited < 10 {
		// Remove: Following is not appropriate here it will be appropriate where it is getting called
...
func GetDaemonset(daemonsetName string) (v1beta1.DaemonSet, error) {
	clientset, err := GetClientset()

	daemonsetClient := clientset.ExtensionsV1beta1().DaemonSets(core_v1.NamespaceDefault)
